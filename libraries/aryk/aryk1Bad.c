...
/* Local variables: */
   ...
   hdsdim lmtr[ARYK__MXDIM];   /* Lower mapping transfer region bounds */
   hdsdim umrb[ARYK__MXDIM];   /* Upper mapping region bounds */
   hdsdim umtr[ARYK__MXDIM];   /* Upper mapping transfer region bounds */
...
/* If the bad pixel flag is still not 1, then a further check must be
   ...
   entry, as if the array were going to be mapped. */
            if( !bad ){
               aryk1Gmrb( acb, &mtrex, &mrfull, &whole, lmrb, umrb, lmtr,
                         umtr, status );
...
/* Call error tracing routine and exit. */
   if( *status != SAI__OK ) aryk1Trace( "aryk1Bad", status );
