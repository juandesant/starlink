...
/* If this pointer is non-NULL, then the array is currently mapped for
   ...
   if( mcb ){
      aryk1Bpp( mcb->type, el, mcb->dpntr, bad, status );
      if( *status == SAI__OK ){
...
/* If the array is mapped for access to complex data, and no bad pixels
   ...
   must be checked in the same way. */
         if( mcb->complex && ( !*bad ) ){
            aryk1Bpp( mcb->type, el, mcb->ipntr, bad, status );
         }
      }
...
/* Otherwise, map the array for read access. */
            ...
            aryk1Maps( acb, dcb->type, dcb->complex, "READ", NULL, &dpntr,
                      &ipntr, status );
...
/* Examine the non-imaginary mapped data for bad pixels. */
            aryk1Bpp( dcb->type, el, dpntr, bad, status );
            if( *status == SAI__OK ){
...
/* If the array is complex, and no bad pixels have yet been found, then the
   ...
   way. */
               if( dcb->complex && ( !bad ) ){
                  aryk1Bpp( dcb->type, el, ipntr, bad, status );
               }
            }

/* Unmap the array. */
            aryk1Umps( acb, status );
         }
      }
   }

/* Call error tracing routine and exit. */
   if( *status != SAI__OK ) aryk1Trace( "aryk1Chbpp", status );
