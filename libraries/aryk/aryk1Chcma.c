...
/* Local variables: */
   ...
   char umode[ARYK__SZMOD+1];  /* Upper case access mode string */
...
/* If there may still be a conflict, then see if the contending mapping
   transfer regions overlap spatially. */
                     aryk1Xsbnd( ARYK__MXDIM, mcb->lmtr, mcb->umtr,
                                ARYK__MXDIM, mcbt->lmtr, mcbt->umtr,
...
/* Call error tracing routine and exit. */
   if( *status != SAI__OK ) aryk1Trace( "aryk1Chcma", status );
