...
/* Local variables: */
   char umode[ARYK__SZACC+1];  /* Upper case version of MODE */
...
/* Report an error if the MODE value supplied is not recognised. */
      ...
      *status = ARYK__FATIN;
      ...
      errRep( " ", "Routine aryk1Chmod called with an invalid MODE argument of"
              "'^BADMODE' (internal programming error).", status );
   }

/* Call error tracing routine and exit. */
   if( *status != SAI__OK ) aryk1Trace( "aryk1Chmod", status );
