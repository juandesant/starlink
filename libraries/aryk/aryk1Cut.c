...
void aryk1Cut( AryACB *acb1, int ndim, const hdsdim *lbnd, const hdsdim *ubnd,
...
*  Synopsis:
*     void aryk1Cut( AryACB *acb1, int ndim, const hdsdim *lbnd,
...
/* Local variables: */
   ...
   hdsdim lx[ARYK__MXDIM];     /* Lower intersection region bounds */
   hdsdim ux[ARYK__MXDIM];     /* Upper intersection region bounds */
...
/* Obtain a free slot for the new array in the ACB. */
   *acb2 = aryk1Ffs( ARYK__ACBTYPE, status );
   if( *status == SAI__OK ){
...
/* Otherwise. find the intersection of the new array bounds with the old
   ...
   has access (in the current pixel index system). */
         ...
         aryk1Xsbnd( ARYK__MXDIM, acb1->lbnd, acb1->ubnd,
                    ARYK__MXDIM, (*acb2)->lbnd, (*acb2)->ubnd,
                    ARYK__MXDIM, lx, ux, &exist, status );
         if( *status == SAI__OK ){
...
/* Find the intersection of this region with the data transfer window of
   ...
   bounds and whether it exists in the ACB entry for the new array. */
               aryk1Xsbnd( ARYK__MXDIM, acb1->ldtw, acb1->udtw, ARYK__MXDIM,
                          lx, ux, ARYK__MXDIM, (*acb2)->ldtw, (*acb2)->udtw,
                          &(*acb2)->dtwex, status );
            }
         }
      }
...
/* If there was an error, then release the ACB slot and reset it to NULL. */
      ...
      *acb2 = aryk1Rls( (AryObject *) *acb2, status );
   }

/* Call error tracing routine and exit. */
   if( *status != SAI__OK ) aryk1Trace( "aryk1Cut", status );
