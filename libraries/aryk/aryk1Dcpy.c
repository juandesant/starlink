...
*  Synopsis:
*     void aryk1Dcpy( AryDCB *dcb1, int temp, HDSLoc **loc, AryDCB **dcb2,
*                    int *status )
...
/* Obtain a new slot in the DCB, resetting the dcb2 argument to NULL if
   ...
   *dcb2 = (AryDCB *) aryk1Ffs( ARYK__DCBTYPE, status );
   if( *status != SAI__OK ){
      *dcb2 = NULL;
   } else {

/* Ensure the data object arrays are available. */
      aryk1Dobj( dcb1, status );
   ...
   NULL. */
      if( *status != SAI__OK ){
         if( dcb2 ){
            ...
            *dcb2 = aryk1Rls( (AryObject *) *dcb2, status );
         }
...
/* Call error tracing routine and exit. */
   if( *status != SAI__OK ) aryk1Trace( "aryk1Dcpy", status );
