...
void aryk1Dcrep( int defer, const char *type, int ndim, const hdsdim *ubnd,
...
/* Obtain a free slot in the DCB. Return a NULL DCB pointer if no slot
   could be found. */
   *dcb = (AryDCB *) aryk1Ffs( ARYK__DCBTYPE, status );
   if( *status != SAI__OK ){
      *dcb = NULL;
...
/* If we are defering creation of the array, create an ARRAY structure
   ...
   "PRIMITIVE". This is picked up by the aryk1Dfrm function, and used as an
   indication that the array is primitive (actually, will be primitive
   when it is created). */
      } else {
         aryk1Dfppl( locp, name, loc, status );
      }
...
/* Release the allocated DCB slot. */
         *dcb = aryk1Rls( (AryObject *) *dcb, status );
...
/* Call error tracing routine and exit. */
   if( *status != SAI__OK ) aryk1Trace( "aryk1Dcrep", status );
