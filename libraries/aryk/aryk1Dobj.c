...
/* Local variables: */
   ...
   hdsdim dim[ARYK__MXDIM];      /* Dimensions of array */
...
/* Do nothing if the creation of the arrays is not deferred. */
   if( aryk1Defr( dcb, status ) && *status == SAI__OK ){
...
/* Report an error if required information is not available. The only
   ...
   set it up. */
      if( !dcb->ktype ){
         *status = ARYK__UNDEF;
         errRep( " ", "aryk1Dobj: Deferred creation of an array is not possible"
...
/* Report an error since delta arrays can only be created by converting a
   ...
   deferred. */
         if( *status == SAI__OK ){
            *status = ARYK__FATIN;
            errRep( " ", "aryk1Dobj: Input array is stored in DELTA form, "
...
/* Call error tracing routine and exit. */
   if( *status != SAI__OK ) aryk1Trace( "aryk1Dobj", status );
