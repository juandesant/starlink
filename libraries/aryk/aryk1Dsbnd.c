...
void aryk1Dsbnd( int ndim, const hdsdim *lbnd, const hdsdim *ubnd, AryDCB *dcb,
...
*  Synopsis:
*     void aryk1Dsbnd( int ndim, const hdsdim *lbnd, const hdsdim *ubnd,
...
/* Change the array bounds, possibly obtaining a new data object locator as
   a result. */
               aryk1Rebnd( defer, locp, name, dcb->type, dcb->state, dcb->ndim,
...
/* Derive a new non-imaginary component locator by cloning the data object
   ...
   flags a deferred array (see aryk1Defr). */
...
/* Otherwise, change the bounds of the non-imaginary data component in what
   is now a simple array. */
                  aryk1Rebnd( defer, dcb->loc, "DATA", dcb->type, dcb->state,
...
aryk1S2dlt is riddled with places that assumes INTEGER rather than K and
...
/* Change the bounds of the non-imaginary data component. */
         aryk1Rebnd( defer, dcb->loc, "DATA", dcb->type, dcb->state, dcb->ndim,
...
/* If the array holds complex data, then change the bounds of the imaginary
   data component. */
         if( dcb->complex ){
            aryk1Rebnd( defer, dcb->loc, "IMAGINARY_DATA", dcb->type,
...
/* Call error tracing routine and exit. */
   if( *status != SAI__OK ) aryk1Trace( "aryk1Dsbnd", status );
