...
/* Report an error if the creation of the data arrays has been deferred.
   The only situation in which creation is deferred is if aryk1Dcre(p) is
   called with its "defer" parameter set non-zero. In this case, all the
   required information should already be available because aryk1Dcre(p)
   will have set it up. */
      if( aryk1Defr( dcb, status ) ){
         *status = ARYK__UNDEF;
...
/* If the type is not numeric, then report an error. */
            numer = aryk1Intyp( dcb->type, status );
            if( *status == SAI__OK ){
               if( !numer ){
                  *status = ARYK__TYPIN;
...
/* Call error tracing routine and exit. */
   if( *status != SAI__OK ) aryk1Trace( "aryk1Dtyp", status );
