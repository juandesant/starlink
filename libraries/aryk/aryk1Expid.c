...
void *aryk1Expid( AryObject *object, int *status ) {
/*
*+
*  Name:
*     aryk1Expid
...
/* Chack that the supplied Object is valid and report an
   error if it is not. */
   if( !aryk1IsValid( object, status ) ) {
      *status = ARYK__FATIN;
      errRep( " ", "Function aryk1Expid called with an invalid pointer "
              " (internal programming error).", status );

/* Check that the object is an ACB or PCB and not a DCB, etc. */
   } else if( object->type != ARYK__ACBTYPE &&
              object->type != ARYK__PCBTYPE ) {
      *status = ARYK__FATIN;
      errRep( " ", "Pointer supplied to aryk1Expid is of an inappropriate "
