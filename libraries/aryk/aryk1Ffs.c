...
*        ARYK__ACBTYPE, ARYK__DCBTYPE, ARYK__MCBTYPE and ARYK__PCBTYPE are
...
*  Notes:
*     -  The returned pointer should be released using aryk1Rls when it is
*     no longer needed.
...
/* Report an error if the array could not be extended. */
         if( *status != SAI__OK ) {
            errRepf( "", "aryk1Ffs: Failed to extend array of %s objects.",
                    status, name );
...
/* Call error tracing routine and exit. */
   if( *status != SAI__OK ) aryk1Trace( "aryk1Ffs", status );
