...
void aryk1Gtn( int bad, const char *htype, HDSLoc *loc, int ndim,
...
/* Local variables: */
   char type[ARYK__SZTYP+1];    /* Upper case data type for output array */
...
/* If the supplied string is not too long, convert it to upper case. */
   typok =  ( strlen(atype) <= ARYK__SZTYP );
   if( typok ) {
      astChrCase( atype, type, 1, ARYK__SZTYP + 1 );
...
/* Test the output data type string against each permitted value in turn,
   calling the appropriate routine to extract the data subregion. */
      if( !strcmp( type,  "_BYTE" ) ){
         aryk1GtnB( bad, htype, loc, ndim, lbndd, ubndd, lsub, usub, lbnda,
                   ubnda, pad, scloc, pntr, dce, status );

      } else if( !strcmp( type,  "_UBYTE" ) ){
         aryk1GtnUB( bad, htype, loc, ndim, lbndd, ubndd, lsub, usub, lbnda,
                    ubnda, pad, scloc, pntr, dce, status );

      } else if( !strcmp( type,  "_DOUBLE" ) ){
         aryk1GtnD( bad, htype, loc, ndim, lbndd, ubndd, lsub, usub, lbnda,
                   ubnda, pad, scloc, pntr, dce, status );

      } else if( !strcmp( type,  "_INTEGER" ) ){
         aryk1GtnI( bad, htype, loc, ndim, lbndd, ubndd, lsub, usub, lbnda,
                   ubnda, pad, scloc, pntr, dce, status );

      } else if( !strcmp( type,  "_REAL" ) ){
         aryk1GtnF( bad, htype, loc, ndim, lbndd, ubndd, lsub, usub, lbnda,
                   ubnda, pad, scloc, pntr, dce, status );

      } else if( !strcmp( type,  "_WORD" ) ){
         aryk1GtnW( bad, htype, loc, ndim, lbndd, ubndd, lsub, usub, lbnda,
                   ubnda, pad, scloc, pntr, dce, status );

      } else if( !strcmp( type,  "_UWORD" ) ){
         aryk1GtnUW( bad, htype, loc, ndim, lbndd, ubndd, lsub, usub, lbnda,
                    ubnda, pad, scloc, pntr, dce, status );

      } else if( !strcmp( type,  "_INT64" ) ){
         aryk1GtnK( bad, htype, loc, ndim, lbndd, ubndd, lsub, usub, lbnda,
                   ubnda, pad, scloc, pntr, dce, status );
...
/* If the string supplied for the "atype" argument is not valid, then report
   ...
   if( !typok && *status == SAI__OK ){
      *status = ARYK__FATIN;
      ...
      errRep( " ", "Function aryk1Gtn called with an invalid 'atype' "
              "argument of '^BADATYPE' (internal programming error).",
              status );
   }

/* Call error tracing routine and exit. */
   if( *status != SAI__OK ) aryk1Trace( "aryk1Gtn", status );
