...
void aryk1Maps( AryACB *acb, const char *type, int cmplx, const char *mmod,
...
*  Synopsis:
*     void aryk1Maps( AryACB *acb, const char *type, int cmplx,
...
/* Local variables: */
   ...
   char mode[ARYK__SZMOD+1];   /* Mapping mode buffer */
   char tinop[ARYK__SZIOP+1];  /* Temporary initialisation option buffer */
   char vtype[ARYK__SZTYP+1];  /* Validated data type string */
   hdsdim dim[ARYK__MXDIM];    /* Array (ACB) dimension sizes */
...
/* Case 1. ======= If the mapping mode is READ or UPDATE and the array's
   ...
   non-imaginary array component for reading. */
            if( ( !strcmp( mode, "READ" ) || !strcmp( mode, "UPDATE" ) ) &&
                dcb->state ){
               bad = acb->bad;
               aryk1Mpsr( acb, dcb->dloc, vtype, mode, bad, &mcb->dloc,
   ...
   occurs. */
               if( *status == SAI__OK ){
                  if( cmplx ){
                     if( dcb->complex ){
                        idce = 0;
                        aryk1Mpsr( acb, dcb->iloc, vtype, mode, bad,
...
/* If access to an imaginary component is required, but such a component
   ...
   object and fill it with zeros. */
                        ...
                        mcb->icopy = 1;
                        aryk1Cmtmp( vtype, acb->ndim, dim, &mcb->iloc,
                                   &mcb->ipntr, status );
                        aryk1Vzero( vtype, el, mcb->ipntr, status );
                     }
                  }
...
/* If the data object's values have not yet been initialised, then see if
   the region being mapped covers the entire object. */
               if( !dcb->init ){
                  aryk1Inbnd( ARYK__MXDIM, mcb->lmtr, mcb->umtr, dcb->ndim,
...
/* Now obtain the required access to the data object; map the non-imaginary
   array component for writing. */
               aryk1Mpsw( acb, dcb->dloc, vtype, inopt, &mcb->dloc,
...
/* If access to the imaginary component is required and such a component
   exists, then map it for writing. */
               if( *status == SAI__OK ){
                  if( cmplx ){
                     if( dcb->complex ){
                        aryk1Mpsw( acb, dcb->iloc, vtype, inopt, &mcb->iloc,
...
/* If access to an imaginary component is required, but such a component
   ...
   option. */
                        ...
                        aryk1Cmtmp( vtype, acb->ndim, dim, &mcb->iloc,
                                   &mcb->ipntr, status );
                        aryk1Iobw( vtype, inopt, el, mcb->ipntr, status );
                     }
                  }
...
/* Create and map a temporary object and initialise its values as required
   by the initialisation option. */
               mcb->dcopy = 1;
               aryk1Cmtmp( vtype, acb->ndim, dim, &mcb->dloc, &mcb->dpntr,
                          status );
               aryk1Iobw( vtype, inopt, el, mcb->dpntr, status );

/* If access to an imaginary component is also required, then repeat this
   process. */
               if( cmplx ){
                  mcb->icopy = 1;
                  aryk1Cmtmp( vtype, acb->ndim, dim, &mcb->iloc, &mcb->ipntr,
                             status );
                  aryk1Iobw( vtype, inopt, el, mcb->ipntr, status );
               }
...
/* Store the mapping access type and mode information in the MCB. */
            aryk1Ccpy( vtype, sizeof(mcb->type), mcb->type, status );
            mcb->complex = cmplx;
            aryk1Ccpy( mode, sizeof(mcb->amm), mcb->amm, status );
...
/* If mapping was not successful, then clear the MCB index from the ACB
   (indicating that the array is not mapped) and release the MCB slot. */
            ...
            acb->mcb = NULL;
            mcb = aryk1Rls( (AryObject *) mcb, status );
         }
      }
   }

/* Call error tracing routine and exit. */
   if( *status != SAI__OK ) aryk1Trace( "aryk1Maps", status );
