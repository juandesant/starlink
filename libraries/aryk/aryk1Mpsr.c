...
void aryk1Mpsr( AryACB *acb, HDSLoc *loc, const char *type,
...
/* Local variables: */
   ...
   hdsdim dim[ARYK__MXDIM];    /* Dimensions of mapping region */
   hdsdim diml[ARYK__MXDIM];   /* Lower bounds of slice */
   hdsdim dimu[ARYK__MXDIM];   /* Upper bounds of slice */
...
/* Otherwise, if the mapping transfer region fills the mapping region and
   ...
   } else if( mcb->mrful && ( ndimd <= ARYK__MXHSL ) &&
...
/* Create and map a temporary object. */
      aryk1Cmtmp( type, ndimd, dim, mloc, pntr, status );
...
/* Uncompress the required section. */
      aryk1Undlt( dcb->loc, ndimd, diml, dimu, *pntr, &newbad, status );
      *copy = 1;
      *dce = 0;
...
/* Create and map a temporary object to hold the output values. */
      aryk1Cmtmp( type, ndima, dim, mloc, pntr, status );
...
/* Create and map a temporary HDS array to hold the uncompressed values. */
         aryk1Cmtmp( extyp, ndimd, dim, &tloc, &tpntr, status );

/* Uncompress the values in the MTR, storing them in the above temporary
   HDS array. */
         aryk1Undlt( dcb->loc, ndimd, diml, dimu, tpntr, &newbad, status );

/* Unmap the temporary array so that the values written into it by
   aryk1Undlt will be available when the array is mapped again within
   aryk1Gtn. */
...
/* Read the data in the mapping transfer region into the returned temporary
   object, padding the rest of the mapped array with "bad" values. */
         aryk1Gtn( bad, dcb->type, loc, ndim, dcb->lbnd, dcb->ubnd, mcb->lmtr,
...
/* Call error tracing routine and exit. */
   if( *status != SAI__OK ) aryk1Trace( "aryk1Mpsr", status );
