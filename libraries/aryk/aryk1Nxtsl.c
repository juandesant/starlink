...
/* These global variables are declared in file aryk1Ffs.c */
...
*  Parameters:
*     type
*        The array to search. The integer symbolic constants ARYK__DCBTYPE,
*        ARYK__ACBTYPE and ARYK__MCBTYPE are available to identify these.
*     slot
...
/* Check the supplied type, and save info about the type. */
   if( type == ARYK__DCBTYPE ){
      start = (AryObject **) Ary_DCB;
      nel = Ary_NDCB;

   } else if( type == ARYK__ACBTYPE ){
      start = (AryObject **) Ary_ACB;
      nel = Ary_NACB;

   } else if( type == ARYK__MCBTYPE ){
...
/* If the common block specified is not valid, then report an error. */
      ...
      *status = ARYK__FATIN;
      msgSeti( "B", (int) type );
      errRep( "ARY1_NXTSL_BBLK", "Function aryk1Nxtsl called with an "
...
/* Call error tracing routine and exit. */
   if( *status != SAI__OK ) aryk1Trace( "aryk1Nxtsl", status );
