...
void aryk1Ptn( int bad, int ndim, const hdsdim *lbnda, const hdsdim *ubnda,
...
/* Test the input data type string against each permitted value in turn,
   ...
   if( !strcasecmp( type, "_BYTE" ) ){
      aryk1PtnB( bad, ndim, lbnda, ubnda, pntr, lsub, usub, lbndd, ubndd,
                htype, loc, dce, status );

   } else if( !strcasecmp( type, "_UBYTE" ) ){
      aryk1PtnUB( bad, ndim, lbnda, ubnda, pntr, lsub, usub, lbndd,
                 ubndd, htype, loc, dce, status );

   } else if( !strcasecmp( type, "_DOUBLE" ) ){
      aryk1PtnD( bad, ndim, lbnda, ubnda, pntr, lsub, usub, lbndd, ubndd,
                htype, loc, dce, status );

   } else if( !strcasecmp( type, "_INTEGER" ) ){
      aryk1PtnI( bad, ndim, lbnda, ubnda, pntr, lsub, usub, lbndd, ubndd,
                htype, loc, dce, status );

   } else if( !strcasecmp( type, "_REAL" ) ){
      aryk1PtnF( bad, ndim, lbnda, ubnda, pntr, lsub, usub, lbndd, ubndd,
                htype, loc, dce, status );

   } else if( !strcasecmp( type, "_WORD" ) ){
      aryk1PtnW( bad, ndim, lbnda, ubnda, pntr, lsub, usub, lbndd, ubndd,
                htype, loc, dce, status );

   } else if( !strcasecmp( type, "_UWORD" ) ){
      aryk1PtnUW( bad, ndim, lbnda, ubnda, pntr, lsub, usub, lbndd,
                 ubndd, htype, loc, dce, status );

   } else if( !strcasecmp( type, "_INT64" ) ){
      aryk1PtnK( bad, ndim, lbnda, ubnda, pntr, lsub, usub, lbndd, ubndd,
                htype, loc, dce, status );
...
/* If the string supplied for the ATYPE argument is not valid, then report
   an error. */
      ...
      *status = ARYK__FATIN;
      ...
      errRep( " ", "Routine aryk1Ptn called with an invalid TYPE argument of"
              "'^BADATYPE' (internal programming error).", status );
   }

/* Call error tracing routine and exit. */
   if( *status != SAI__OK ) aryk1Trace( "aryk1Ptn", status );
