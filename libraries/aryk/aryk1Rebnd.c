...
void aryk1Rebnd( int defer, HDSLoc *paren, const char *name,
...
/* Local variables: */
   ...
   hdsdim dim[ARYK__MXDIM];    /* Object dimensions */
...
/* If the data values are defined, then see if there is any region in
   common between the new array and the old array. */
         ...
         aryk1Xsbnd( ndim, lbnd, ubnd, nndim, nlbnd, nubnd, ARYK__MXDIM, lx,
                    ux, drx, status );
         if( *status == SAI__OK ){
...
/* Create and map workspace to hold this region of data. */
            aryk1Cmtmp( type, mindim, dim, &tloc, &tpntr, status );

/* Read the appropriate part of the old array into this workspace. */
            aryk1Gtn( 0, type, *loc, ndim, lbnd, ubnd, lx, ux, type,
...
/* If a temporary copy of data is being held, then write it back into the
   appropriate region of the new object. Then erase the workspace. */
               if( cpydat ){
                  aryk1Ptn( 0, nndim, lx, ux, type, tpntr, lx, ux,
...
/* Call error tracing routine and exit. */
   if( *status != SAI__OK ) aryk1Trace( "aryk1Rebnd", status );
