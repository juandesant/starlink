...
void aryk1Retyp( HDSLoc *paren, const char *name, const char *type,
...
/* Compare the new data type with each permitted value in turn and call the
   appropriate routine to convert the old data into the new type. */
         if(!strcasecmp( ntype, "_BYTE" ) ){
            aryk1CvtB( bad, el, type, oldptr, pntr, dce, status );

         } else if(!strcasecmp( ntype, "_UBYTE" ) ){
            aryk1CvtUB( bad, el, type, oldptr, pntr, dce, status );

         } else if(!strcasecmp( ntype, "_DOUBLE" ) ){
            aryk1CvtD( bad, el, type, oldptr, pntr, dce, status );

         } else if(!strcasecmp( ntype, "_INTEGER" ) ){
            aryk1CvtI( bad, el, type, oldptr, pntr, dce, status );

         } else if(!strcasecmp( ntype, "_REAL" ) ){
            aryk1CvtF( bad, el, type, oldptr, pntr, dce, status );

         } else if(!strcasecmp( ntype, "_WORD" ) ){
            aryk1CvtW( bad, el, type, oldptr, pntr, dce, status );

         } else if(!strcasecmp( ntype, "_UWORD" ) ){
            aryk1CvtUW( bad, el, type, oldptr, pntr, dce, status );

         } else if(!strcasecmp( ntype, "_INT64" ) ){
            aryk1CvtK( bad, el, type, oldptr, pntr, dce, status );

         } else if( *status == SAI__OK ) {
            *status = ARYK__FATIN;
            errRepf( " ", "aryk1Retyp: Unsupported data type '%s' encountered "
                     "(programming error).", status, ntype );
         }

/* Unmap the new object. */
         aryk1Hunmp( *loc, status );
...
/* Call error tracing routine and exit. */
   if( *status != SAI__OK ) aryk1Trace( "aryk1Retyp", status );
