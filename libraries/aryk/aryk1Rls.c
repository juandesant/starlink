...
/* Get the type of the supplied object - DCB, ACB, MCB or PCB. This also
   ...
   if( object->type == ARYK__DCBTYPE ) {
      type = "DCB";
   } else if( object->type == ARYK__ACBTYPE ) {
      type = "ACB";
   } else if( object->type == ARYK__MCBTYPE ) {
      type = "ACB";
   } else if( object->type == ARYK__PCBTYPE ) {
      type = "ACB";
   } else {
      *status = ARYK__FATIN;
      msgSeti( "T", object->type );
      errRep( " ", "Pointer supplied to function aryk1Rls has "
   ...
   if( !object->used ){
      if( *status == SAI__OK ) {
         *status = ARYK__FATIN;
         ...
         errRep( " ", "Function aryk1Rls called to release an object of "
