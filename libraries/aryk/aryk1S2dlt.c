...
#define MAKE_PROTOA(incode,intype,outcode,outtype) \
   static void aryk1Delt##incode##outcode( intype *pindata, size_t nel, \
                                          size_t stride, outtype *poutdata, \
                                          intype *pvalue, hdsdim *prepeat, \
                                          size_t *ndata, hdsdim *nvalue, \
                                          hdsdim *nrepeat, int *status ); \
\
   static void aryk1Check##incode##outcode( intype *pindata, size_t nel, \
...
void aryk1S2dlt( HDSLoc *loc1, int zaxis, const char *type, HDSLoc *loc2,
...
*     structure.
*
*     See the inverse function, aryk1Undlt.c, for a description of the
...
/* Local Variables; */
   ...
   hdsdim dims_cindata[ ARYK__MXDIM ];
   hdsdim dims_first[ ARYK__MXDIM - 1 ];
   hdsdim dims_indata[ ARYK__MXDIM ];
   hdsdim start[ ARYK__MXDIM ];
   ...
   size_t size_intype;
   size_t size_outtype;
   size_t stride_indata[ ARYK__MXDIM ];
   size_t zstride;
...
/* Finds the dimensions of the array. */
   datShape( loc_indata, ARYK__MXDIM, dims_indata, &ndim, status );
...
#define ASSIGN_FUNS(incode,outcode) \
            delt_fun = (delt_fun_type) aryk1Delt##incode##outcode; \
            check_fun = (check_fun_type) aryk1Check##incode##outcode;
...
/* Report a context error if anything went wrong. */
   if( *status != SAI__OK ) {
      errRep( "", "aryk1S2dlt: Failed to convert a simple array into a delta "
...
*  Invocation:
*     void aryk1Delt<TIN><TOUT>( <TIN> *pindata, size_t nel, size_t stride,
...
*  Invocation:
*     void aryk1Check<TIN><TOUT>( <TIN> *pindata, size_t nel, size_t stride,
