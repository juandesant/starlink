...
/* Local variables: */
   ...
   hdsdim lbad[ARYK__MXDIM];   /* Lower bounds of "bad pixel data" */
   hdsdim lmrb[ARYK__MXDIM];   /* Lower mapping region bounds */
   hdsdim ltest[ARYK__MXDIM];  /* Lower bounds of test region */
   hdsdim lx[ARYK__MXDIM];     /* Lower bounds of intersection region */
   hdsdim ubad[ARYK__MXDIM];   /* Upper bounds of "bad pixel data" */
   hdsdim umrb[ARYK__MXDIM];   /* Upper mapping region bounds */
   hdsdim utest[ARYK__MXDIM];  /* Upper bounds of test region */
   hdsdim ux[ARYK__MXDIM];     /* Upper bounds of intersection region */
...
/* If the ACB has a data transfer window associated with it, then
   ...
   the region of actual data to which the array has access. */
         if( acb->dtwex ){
            mtrex = 0;
            aryk1Gmrb( acb, &mtrex, &mrfull, &whole, lmrb, umrb, lbad, ubad,
                      status );
...
/* If the bad pixel flag is false, then there are no "bad" values in the
   ...
   accordingly if this comprises the whole data object. */
                  ...
                  if( whole ) aryk1Dsbd( 0, dcb, status );
               }
...
/* The effect on other ACB entries, whose mapping transfer regions may
   ...
   thread is currently accessing the slot array. */
               ARYK__ACB_LOCK_MUTEX;
               iacbt = -1;
               next = 0;
               while( 1 ){
                  acbt = aryk1Nxtsl( ARYK__ACBTYPE, iacbt, &next, status );
...
/* Calculate mapping region bounds information for each ACB entry being
   tested. */
                        mtrex = 0;
                        aryk1Gmrb( acbt, &mtrex, &mrfull, &whole, lmrb, umrb,
                                  ltest, utest, status );
...
/* If the bad pixel flag is true, then data associated with the ACB
   ...
   set its flag accordingly. */
                           if( bad ){
                              insect = 0;
                              aryk1Xsbnd( ARYK__MXDIM, lbad, ubad, ARYK__MXDIM,
                                         ltest, utest, ARYK__MXDIM, lx, ux,
                                         &insect, status );
...
/* If the bad pixel flag is false, then we can only be sure there are no
   ...
   ACB. Test if this is so, and set its flag accordingly. */
                              ...
                              inside = 0;
                              aryk1Inbnd( ARYK__MXDIM, lbad, ubad, ARYK__MXDIM,
...
/* Call error tracing routine and exit. */
   if( *status != SAI__OK ) aryk1Trace( "aryk1Sbd", status );
