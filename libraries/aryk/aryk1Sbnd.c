...
void aryk1Sbnd( int ndim, const hdsdim *lbnd, const hdsdim *ubnd, AryACB *acb,
               int *status ) {
/*
*+
*  Name:
...
*  Synopsis:
*     void aryk1Sbnd( int ndim, const hdsdim *lbnd, const hdsdim *ubnd,
...
/* Local variables: */
   ...
   hdsdim ldtw[ARYK__MXDIM];   /* New lower data transfer window bounds */
   hdsdim lx[ARYK__MXDIM];     /* Lower bounds of retained data region */
   hdsdim udtw[ARYK__MXDIM];   /* New upper data transfer window bounds */
   hdsdim ux[ARYK__MXDIM];     /* Upper bounds of retained data region */
...
/* If a data transfer window exists, then find the intersection of this
   ...
   pixel index system). Note whether a data transfer window now exists. */
         if( acb->dtwex ){
            aryk1Xsbnd( ARYK__MXDIM, ldtw, udtw, ARYK__MXDIM, acb->ldtw,
                       acb->udtw, ARYK__MXDIM, acb->ldtw, acb->udtw,
                       &acb->dtwex, status );
         }
      }
...
/* Set the new bounds for the data object. */
         ...
         aryk1Dsbnd( ndim, lbnd, ubnd, dcb, &same, &drx, lx, ux, status );
         if( *status == SAI__OK ){
...
/* Call error tracing routine and exit. */
   if( *status != SAI__OK ) aryk1Trace( "aryk1Sbnd", status );
