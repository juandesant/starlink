...
void aryk1Undlt( HDSLoc *loc1, int ndim_in, const hdsdim *lbnd,
...
*        special flag values (see aryk_dlt.h):
*
...
/* Local Variables; */
   ...
   hdsdim dims_csection[ ARYK__MXDIM ];
   hdsdim dims_cwhole[ ARYK__MXDIM ];
   hdsdim dims_firstd[ ARYK__MXDIM - 1 ];
   hdsdim dims_firstr[ ARYK__MXDIM - 1 ];
   hdsdim dims_firstv[ ARYK__MXDIM - 1 ];
   hdsdim dims_out[ ARYK__MXDIM ];
   hdsdim dims_section[ ARYK__MXDIM ];
   hdsdim dlb;
   hdsdim dub;
   hdsdim lbnd_csection[ ARYK__MXDIM ];
   hdsdim lbnd_cwhole[ ARYK__MXDIM ];
   hdsdim start[ ARYK__MXDIM + 1 ];
   hdsdim ubnd_csection[ ARYK__MXDIM + 1 ];
   hdsdim *prepeat;
   ...
   int zaxis;
   hdsdim zdim;
   size_t div_cwhole[ ARYK__MXDIM ];
   size_t div_section[ ARYK__MXDIM ];
   size_t iv_csection;
   size_t iv_cwhole;
   size_t iv_section;
   ...
   size_t size_outtype;
   size_t size_vtype;
   size_t stride_cwhole[ ARYK__MXDIM ];
   size_t stride_section[ ARYK__MXDIM ];
   size_t zstride;
...
/* Get the number and lengths of the other axes. These are defined by the
   FIRST_DATA array. */
   datShape( loc_firstd, ARYK__MXDIM - 1, dims_firstd, &ndim, status );
...
/* Report an error if the dimensionality of the supplied bounds is
   ...
   if( ndim != ndim_in && *status == SAI__OK ) {
      *status = ARYK__FATIN;
      ...
      msgSeti( "N", ndim_in );
      msgSeti( "M", ndim );
      errRep( "", "aryk1Undlt: error uncompressed array '^A' - the "
...
#define CHOOSE_FUNB(outcode,vcode) \
         if( !strcmp( type_data, "_INTEGER" ) ) { \
            ...
            *status = ARYK__FATIN; \
            msgSetc( "T", type_data ); \
            errRep( "", "aryk1Undlt: Unsupported input data type '^T' " \
                    "(programming error).", status ); \
            goto L999; \
         }
...
#define CHOOSE_FUNC(vcode) \
      if( !strcmp( type, "_DOUBLE" ) ) { \
         ...
         *status = ARYK__FATIN; \
         msgSetc( "T", type ); \
         errRep( "", "aryk1Undlt: Unsupported output data type '^T' " \
                 "(programming error).", status ); \
         goto L999; \
      }
   ...
   if( !strcmp( type_value, "_INTEGER" ) ) {
      ...
      *status = ARYK__FATIN;
      msgSetc( "T", type );
      errRep( "", "aryk1Undlt: Unsupported VALUE data type '^T' "
              "(programming error).", status );
      goto L999;
   }
...
/* We also know we need the FIRST_VALUE array, so find it, map it, and
   report an error if it has a different shape to the FIRST_DATA array. */
      ...
      datShape( loc_firstv, ARYK__MXDIM - 1, dims_firstv, &ndim_firstv,
                status );
...
/* We also need the FIRST_REPEAT array but only if the REPEAT array exist.
   ...
   to the FIRST_DATA array. */
      if( loc_repeat ) {
         ...
         datShape( loc_firstr, ARYK__MXDIM - 1, dims_firstr, &ndim_firstr,
                   status );
...
/* Report a context error if anything went wrong. */
   if( *status != SAI__OK ) {
      errRep( "", "aryk1Undlt: Failed to uncompress a section of a "
...
/* Report an error if the previous value is unknown. */ \
         ...
         *status = ARYK__FATIN; \
         errRep( "", "aryk1Undlt: First compressed value is a delta " \
                 "(programming error).", status ); \
...
\
         ...
         *status = ARYK__FATIN; \
         errRep( "", "aryk1Undlt: Inconsistent over-shoot (programming " \
                 "error).", status ); \
         return; \
      } \
   } \
