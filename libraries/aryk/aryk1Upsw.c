...
void aryk1Upsw( AryACB *acb, const char *type, int bad, int copy,
...
/* If a mapping transfer region exists, then write back the data lying
   inside this region. */
      if( mcb->mtrex ){
         dcb = acb->dcb;
         aryk1Ptn( bad, ( acb->ndim > dcb->ndim ) ? acb->ndim : dcb->ndim,
