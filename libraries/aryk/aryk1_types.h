#if !defined( ARYK1_TYPES_INCLUDED )   /* Protect against multiple inclusion*/
#define ARYK1_TYPES_INCLUDED 1
...
*  Purpose:
*     Defines private data types and constants used by ARYK.
...
/* Macros to lock and unlock a mutex that serialises access to the
   global variables used byu aryk1Nxtsl (and declared in aryk1Ffs). These
   mutexes should be locked by any function that uses aryk1Nxtsl to search
   the list of ACB, DCB, PCB or MCP entries, or which accesses these
   lists directly. */

extern pthread_mutex_t Ary_ACB_mutex;
#define ARYK__ACB_LOCK_MUTEX pthread_mutex_lock( &Ary_ACB_mutex );
#define ARYK__ACB_UNLOCK_MUTEX pthread_mutex_unlock( &Ary_ACB_mutex );

extern pthread_mutex_t Ary_DCB_mutex;
#define ARYK__DCB_LOCK_MUTEX pthread_mutex_lock( &Ary_DCB_mutex );
#define ARYK__DCB_UNLOCK_MUTEX pthread_mutex_unlock( &Ary_DCB_mutex );

extern pthread_mutex_t Ary_PCB_mutex;
#define ARYK__PCB_LOCK_MUTEX pthread_mutex_lock( &Ary_PCB_mutex );
#define ARYK__PCB_UNLOCK_MUTEX pthread_mutex_unlock( &Ary_PCB_mutex );

extern pthread_mutex_t Ary_MCB_mutex;
#define ARYK__MCB_LOCK_MUTEX pthread_mutex_lock( &Ary_MCB_mutex );
#define ARYK__MCB_UNLOCK_MUTEX pthread_mutex_unlock( &Ary_MCB_mutex );


/* Maximum number of dimensions for which the data system (HDS) is
   capable of "slicing" a primitive object. */
#define ARYK__MXHSL 3

/* Unique ARYK_ system facility number. */
#define ARYK__FACNO 1503

/* Maximum length of a string describing an active mapping mode entry
   in the MCB, e.g. 'UPDATE'. */
#define ARYK__SZAMM 6

/* Maximum length of a string describing a data object disposal mode,
   entry in the DCB, e.g. 'DELETE'. */
#define ARYK__SZDSP 6
...
typedef enum AryAccessControlFlag  {
  ARYK__ACC_BOUND = BIT_TO_VAL(0), /* BOUNDS access control flag. */
  ARYK__ACC_DELET = BIT_TO_VAL(1), /* DELETE access control flag. */
  ARYK__ACC_SHIFT = BIT_TO_VAL(2), /* SHIFT access control flag. */
  ARYK__ACC_TYPE  = BIT_TO_VAL(3), /* TYPE access control flag. */
  ARYK__ACC_WRITE = BIT_TO_VAL(4), /* WRITE access control flag. */
  ARYK__ACC_SCALE = BIT_TO_VAL(5)  /* SCALE access control flag. */
...
/* Slot. The index of the block structure within the arrays maintained
   by aryk1Ffs. For historical reasons, this is referred to as the slot
...
/* Check: When an integer identifier is issued for an object, its value is
   ...
   ARYK_  system by assigning NULL to them. */
   int check;
...
/* Define a structure to hold information about a single physical data
   object (usually a disk file) handled by the ARYK_ system.  A one-to-one
   ...
   of the ARYK library, where "block" refered to a Fortran common block.
   The "DCB" was a common block contining information about all known
   data objects (each one described by a "slot" in the "DCB"). By
   contract, in the C version of the ARYK library a "DCB" is a single
...
/* Disposal mode: If the DCB is in use, then the "dispose" value contains the
   ...
   be taken when the associated data object is released from the ARYK_
   system. Release occurs when the "refcount" value for the object falls to
   zero. */
   char  dispose[ ARYK__SZDSP + 1 ];
...
/* Dimensionality and bounds information: If the DCB is in use and the
   ...
   padded with 1's to make the number of bounds up to ARYK__MXDIM. If
...
/* Accumulated pixel index shifts: If the DCB is in use, then the "shift"
   ...
   to the ARYK_ system. A total of ARYK__MXDIM shifts is held, regardless of
...
/* Container file and HDS path names: If the DCB is in use, then the
   ...
   char file[ ARYK__SZFIL + 1 ];
   char path[ ARYK__SZPTH + 1 ];
...
/* Mapping region bounds: If the MCB is in use, then the "lmrb" and "umrb"
   ...
   values are padded with 1's to give ARYK__MXDIM bounds, regardless of the
   actual data object dimensionality. */
   hdsdim lmrb[ ARYK__MXDIM ];
   hdsdim umrb[ ARYK__MXDIM ];
...
/* Mapping transfer region: If the MCB is in use, then the "mtrex" value
   ...
   give ARYK__MXDIM bounds, regardless of the data object's actual
   dimensionality. */
   int mtrex;
   hdsdim lmtr[ ARYK__MXDIM ];
   hdsdim umtr[ ARYK__MXDIM ];
...
/* Define a structure to hold information about the "virtual"
   objects used by the ARYK_ system to provide user access to actual
...
/* Dimensionality and bounds information: If the ACB is in use, then the
   ...
   give ARYK__MXDIM bounds, regardless of the actual object dimensionality. */
   int ndim;
   hdsdim lbnd[ ARYK__MXDIM ];
   hdsdim ubnd[ ARYK__MXDIM ];
...
/* Data transfor window: If the ACB is in use, then the "dtwex" value
   ...
   always has ARYK__MXDIM dimensions and is stored using the "reference
   frame" pixel index system (this is the pixel index system which the
   actual data object had when it first became known to the ARYK_ system).
   ...
   int dtwex;
   hdsdim ldtw[ ARYK__MXDIM ];
   hdsdim udtw[ ARYK__MXDIM ];
...
/* Define a structure to hold information about placeholders used by
   the ARYK_ system. Objects in this class are kept in one-to-one
...
/* Placeholder locator:  If the PCB is in use, then "loc is an HDS locator
   to an ARYK_ placeholder, which reserves a position in the data system. */
   HDSLoc *loc;
