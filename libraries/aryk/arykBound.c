...
void arykBound( Ary *ary, int ndimx, hdsdim *lbnd, hdsdim *ubnd, int *ndim,
               int *status ) {
/*
*+
*  Name:
...
*     exceeds 1.
*     -  The symbolic constant ARYK__MXDIM may be used to declare the
...
*     return. This constant is defined in the header file "aryk.h".
...
/* Import the array identifier. */
   acb = (AryACB *) aryk1Impid( ary, 1, 1, 1, status );
   if( *status == SAI__OK ){
...
/* If an error occurred, then report context information and call the error
   tracing routine. */
   if( *status != SAI__OK ){
      errRep( " ", "arykBound: Error obtaining the pixel-index bounds of an"
              "array.", status );
      aryk1Trace( "arykBound", status );
   }
