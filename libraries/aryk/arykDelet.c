...
*     function is equivalent to calling arykAnnul.
...
/* Report context information and call error tracing routine if
   appropriate. */
         ...
         errRep( " ", "arykDelet: Error deleting an array.", status );
         aryk1Trace( "arykDelet", status );
      }
   } else {
      *status = tstat;
   }
