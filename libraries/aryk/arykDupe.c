...
void arykDupe( Ary *ary1, AryPlace **place, Ary **ary2, int *status ) {
/*
*+
*  Name:
*     arykDupe
...
*     place
*        An array placeholder (e.g. generated by the arykPlace routine)
...
/* Create a new data object with the same attributes and an entry in the
   ...
   array. */
                  if( pbnd ){
                     aryk1Dcrep( 1, dcb1->type, acb1->ndim, acb1->ubnd,
                                pcb->tmp, &pcb->loc, &dcb2, status );
                  } else {
                     aryk1Dcre( 1, dcb1->type, dcb1->complex, acb1->ndim,
...
/* Report context information and call the error tracing routine if
   appropriate. */
         ...
         errRep( " ", "arykDupe: Error duplicating an array.", status );
         aryk1Trace( "arykDupe", status );
      }
   } else {
      *status = tstat;
   }
