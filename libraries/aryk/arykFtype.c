...
*  Notes:
*     -  The symbolic constant ARYK__SZFTP should be used for declaring the
*     length of a character variable to hold the full data type of an
*     array. This constant is defined in the header file aryk.h.
...
/* If an error occurred, then report context information and call the error
   tracing routine. */
   if( *status != SAI__OK ){
      errRep( " ", "arykFtype: Error obtaining the full data type of an array.",
              status );
      aryk1Trace( "arykFtype", status );
   }
