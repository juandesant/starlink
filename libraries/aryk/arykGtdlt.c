...
void arykGtdlt( Ary *ary, int *zaxis, char ztype[DAT__SZTYP+1], float *zratio,
               int *status ) {
/*
*+
*  Name:
...
*  Synopsis:
*     void arykGtdlt( Ary *ary, int *zaxis, char ztype[DAT__SZTYPE+1],
...
/* Get the compression information from the data object. */
   aryk1Gtdlt( dcb, zaxis, ztype, zratio, status );

/* If an error occurred, then report context information and call the error
   tracing routine. */
   if( *status != SAI__OK ){
      errRep( " ", "arykGtdlt: Error getting information about a delta "
              "compressed array.", status );
      aryk1Trace( "arykGtdlt", status );
   }
