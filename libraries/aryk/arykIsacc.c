...
void arykIsacc( Ary *ary, const char access[ARYK__SZACC+1], int *isacc,
               int *status ) {
/*
*+
*  Name:
...
/* Determine whether access is available. */
   aryk1Accok( acb, access, isacc, status );

/* If an error occurred, then report context information and call the error
   tracing routine. */
   if( *status != SAI__OK ){
      errRep( " ", "arykIsacc: Error enquiring whether access to an array is"
              " available.", status );
      aryk1Trace( "arykIsacc", status );
   }
