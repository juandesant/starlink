...
*  Synopsis:
*     arykLock( Ary *ary, int readonly, int *status );
...
*     arykLock, datUnlock and arykLocked.
...
/* Attempt to lock the specified array. An error will be reported if the
   ...
   thread. */
      aryk1DCBLock( acb->dcb, 2, readonly, status );
   }
...
/* If an error occurred, then report context information and call the
   ...
   if( *status != SAI__OK ) {
      ...
      errRep( "ARY_LOCK_ERR", "arykLock: Error locking an array for ^U "
              "access by the current thread.", status );
      aryk1Trace( "ary1Lock", status );
   }
}
