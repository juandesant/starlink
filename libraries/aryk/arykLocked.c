...
*  Synopsis:
*     locked = arykLocked( const Ary *ary, int *status );
...
*     The lock management functions (arykLock and arykUnlock) will ensure
...
*     read-write lock using arykUnlock. If a thread requests and is given
...
*        array for read-write access using function arykLock. This condition
*        can be achieved by releasing any existing locks using arykUnlock.
*
...
/* Import and validate the array identifier, but do not include the usual
   ...
   acb = (AryACB *) aryk1Impid( ary, 0, 0, 1, status );

/* Get the value to return. Test status first so that we know it is safe
   to deference "acb". */
   if( *status == SAI__OK ) {
      result = aryk1DCBLock( acb->dcb, 1, 0, status );
   }

/* If an error occurred, then report context information and call the
   error tracing routine. */
   if( *status != SAI__OK ) {
      errRep( "ARY_LOCKED_ERR", "arykLocked: Error querying the current "
              "locks on an array.", status );
      aryk1Trace( "ary1Locked", status );
   }
