...
void arykMap( Ary *ary, const char *type, const char *mmod, void **pntr,
...
/* Local variables: */
   ...
   char mode[ARYK__SZMOD+1];   /* Data access mode */
   char inopt[ARYK__SZIOP+1];  /* Initialisation option */
...
/* Validate the mapping mode string, decomposing it into an access mode and
   ...
   aryk1Vmmd( mmod, mode, inopt, status );

/* Import the array identifier. */
   acb = (AryACB *) aryk1Impid( ary, 1, !strcmp( mode, "READ" ), 1, status );

/* Map the array data. */
   aryk1Maps( acb, type, 0, mode, inopt, pntr, NULL, status );

/* Check we can de-reference "acb" safely. */
   if( *status == SAI__OK ) {

/* Calculate the number of mapped data elements. */
      aryk1Nel( acb->ndim, acb->lbnd, acb->ubnd, el, status );
   }

/* If an error occurred, then report context information and call the error
   tracing routine. */
   if( *status != SAI__OK ){
      errRep( " ", "arykMap: Error obtaining mapped access to an array.",
              status );
      aryk1Trace( "arykMap", status );
   }
