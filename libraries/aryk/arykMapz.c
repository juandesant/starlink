...
void arykMapz( Ary *ary, const char *type, const char *mmod, void **rpntr,
...
/* Map the array data. */
   aryk1Maps( acb, type, 1, mode, inopt, rpntr, ipntr, status );
...
/* If an error occurred, then report context information and call the error
   tracing routine. */
   if( *status != SAI__OK ){
      errRep( " ", "arykMapz: Error obtaining complex mapped access to an array.",
              status );
      aryk1Trace( "arykMapz", status );
   }
