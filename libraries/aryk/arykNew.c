...
void arykNew( const char *ftype, int ndim, const hdsdim *lbnd, const hdsdim *ubnd,
...
/* Local variables: */
   char type[ARYK__SZTYP+1];   /* Numeric type of the array */
...
/* Create a new simple array structure in place of the placeholder object,
   obtaining a DCB entry which refers to it. */
      if( *status == SAI__OK ){
         aryk1Dcre( 0, type, cmplx, ndim, lbnd, ubnd, pcb->tmp,
                   pcb->loc, &dcb, status );
      }
...
/* If appropriate, report the error context and call the error tracing
   routine. */
         ...
         *ary = NULL;
         errRep( " ", "arykNew: Error creating a new simple array.", status );
         aryk1Trace( "arykNew", status );
      }
   } else {
      *status = tstat;
   }
