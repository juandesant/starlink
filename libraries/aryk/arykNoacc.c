...
/* If an error occurred, then report context information and call the error
   tracing routine. */
   if( *status != SAI__OK ){
      errRep( " ", "arykNoacc: Error disabling access to an array.", status );
      aryk1Trace( "arykNoacc", status );
   }
