...
*  Synopsis:
*     void arykOffs( Ary *ary1, Ary *ary2, int mxoffs, hdsdim *offs,
*                   int *status )
...
*     supplied. The symbolic constant ARYK__MXDIM may be used to declare
...
/* Loop to calculate the offset for each possible array dimension. */
      n = ( mxoffs < ARYK__MXDIM ) ? mxoffs : ARYK__MXDIM;
      for( i = 0; i < n; i++ ){
...
/* If an error occurred, then report context information and call the error
   tracing routine. */
   if( *status != SAI__OK ){
      errRep( " ", "arykOffs: Error obtaining the pixel offset between two"
              " arrays.", status );
      aryk1Trace( "arykOffs", status );
   }
