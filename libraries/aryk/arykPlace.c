...
*     may be passed to other routines (e.g. arykNew) to indicate where
...
/* If an error occurred, then report context information and call the error
   tracing routine. */
   if( *status != SAI__OK ){
      errRep( " ", "arykPlace: Error obtaining array placeholder.", status );
      aryk1Trace( "arykPlace", status );
   }
