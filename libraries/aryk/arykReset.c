...
*     changing its data type with the arykStype function); this will
...
/* Reset the array's state to "undefined". */
            ...
            aryk1Drst( dcb, status );

/* Set its bad pixel flag to .TRUE.. */
            aryk1Sbd( 1, acb, status );
         }
      }
   }
...
/* If an error occurred, then report context information and call the error
   tracing routine. */
   if( *status != SAI__OK ){
      errRep( " ", "arykReset: Error resetting an array to an undefined state.",
              status );
      aryk1Trace( "arykReset", status );
   }
