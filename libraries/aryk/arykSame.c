...
*  Synopsis:
*     void arykSame( Ary *ary1, Ary *ary2, int *same, int *isect,
*                   int *status )
...
/* Local variables: */
   ...
   hdsdim lmrb[ARYK__MXDIM];   /* Lower bounds of mapping region */
   hdsdim lmtr1[ARYK__MXDIM];  /* Lower bounds mapping tr. region 1 */
   hdsdim lmtr2[ARYK__MXDIM];  /* Lower bounds mapping tr. region 2 */
   hdsdim lx[ARYK__MXDIM];     /* Lower bound of intersection region */
   hdsdim umrb[ARYK__MXDIM];   /* Upper bounds of mapping region */
   hdsdim umtr1[ARYK__MXDIM];  /* Upper bounds mapping tr. region 1 */
   hdsdim umtr2[ARYK__MXDIM];  /* Upper bounds mapping tr. region 2 */
   hdsdim ux[ARYK__MXDIM];     /* Upper bound of intersection region */
...
/* Import the two array identifiers. */
   acb1 = (AryACB *) aryk1Impid( ary1, 1, 1, 1, status );
   acb2 = (AryACB *) aryk1Impid( ary2, 1, 1, 1, status );
...
/* If they refer to the same data object, then calculate their mapping
   region bounds, as if they were going to be mapped for access. */
         ...
         aryk1Gmrb( acb1, &mtrex1, &mrful1, &whole1, lmrb, umrb, lmtr1, umtr1,
                   status );
         aryk1Gmrb( acb2, &mtrex2, &mrful2, &whole2, lmrb, umrb, lmtr2, umtr2,
                   status );
         if( *status == SAI__OK ) {
...
/* Otherwise, they intersect if their mapping transfer windows intersect. */
               ...
               aryk1Xsbnd( ARYK__MXDIM, lmtr1, umtr1, ARYK__MXDIM, lmtr2, umtr2,
                          ARYK__MXDIM, lx, ux, isect, status );
            }
         }
      }
   }
...
/* If an error occurred, then report context information and call the
   ...
   if( *status != SAI__OK ) {
      errRep( " ", "arykSame: Error determining if two array identifiers "
              "refer to parts of the same base array.", status );
      aryk1Trace( "arykSame", status );
   }
