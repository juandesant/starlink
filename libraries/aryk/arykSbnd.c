...
void arykSbnd( int ndim, const hdsdim *lbnd, const hdsdim *ubnd, Ary *ary,
              int *status ) {
/*
*+
*  Name:
...
*  Synopsis:
*     void arykSbnd( int ndim, const hdsdim *lbnd, const hdsdim *ubnd,
*                   Ary *ary, int *status )
...
*     arykReset should be made before calling this routine. This will
...
/* Import the array identifier. */
   acb = (AryACB *) aryk1Impid( ary, 1, 0, 1, status );

/* Check that BOUNDS access to the array is available. */
   aryk1Chacc( acb, "BOUNDS", status );

/* Set the new bounds. */
   if( *status == SAI__OK ){
      aryk1Sbnd( ndim, lbnd, ubnd, acb, status );
   }

/* If an error occurred, then report context information and call the error
   tracing routine. */
   if( *status != SAI__OK ){
      errRep( " ", "arykSbnd: Error setting new pixel-index bounds for an "
              "array.", status );
      aryk1Trace( "arykSbnd", status );
   }
