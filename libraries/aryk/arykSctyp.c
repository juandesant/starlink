...
*     using the associated scale and zero values. Use arykType if you
...
*     returns the same type as the arykType function.
*     -  The symbolic constant ARYK__SZTYP should be used to declare the
...
/* If an error occurred, then report context information and call the error
   tracing routine. */
   if( *status != SAI__OK ){
      errRep( " ", "arykSctyp: Error obtaining the numeric type of a scaled"
              " array.", status );
      aryk1Trace( "arykSctyp", status );
   }
