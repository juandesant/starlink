...
void arykSect( Ary *ary1, int ndim, const hdsdim *lbnd, const hdsdim *ubnd,
              Ary **ary2, int *status ) {
/*
*+
*  Name:
...
*  Synopsis:
*     void arykSect( Ary *ary1, int ndim, const hdsdim *lbnd,
...
/* Create an ACB entry for the new array. */
   if( *status == SAI__OK ){
      aryk1Cut( acb1, ndim, lbnd, ubnd, &acb2, status );
   }

/* Export an identifier for the new array. */
   *ary2 = aryk1Expid( (AryObject *) acb2, status );
...
/* If an error occurred, then set a value of NULL for the "ary2"
   ...
   if( *status != SAI__OK ){
      *ary2 = NULL;
      errRep( " ", "arykSect: Error creating an array section.",
              status );
      aryk1Trace( "arykSect", status );
   }
