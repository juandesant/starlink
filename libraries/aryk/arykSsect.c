...
/* Local variables: */
   ...
   hdsdim lbnd[ARYK__MXDIM];   /* Lower bound of region to be accessed */
   hdsdim lx[ARYK__MXDIM];     /* Lower bounds of data transfer window */
   hdsdim sft[ARYK__MXDIM];    /* Differential pixel index shifts */
   hdsdim ubnd[ARYK__MXDIM];   /* Upper bound of region to be accessed */
   hdsdim ux[ARYK__MXDIM];     /* Upper bounds of data tranfer window */
...
/* If either input array is not a base array, or if the bounds do not match
   ...
   it. */
      if( acb1->cut || acb2->cut || !match ){
         aryk1Cut( acb1, acb1->ndim, lbnd, ubnd, &acb3, status );
         if( *status == SAI__OK ){
...
/* Otherwise, find the intersection region between the current output array
   ...
   region exists in the ACB entry for the output array. */
                  ...
                  aryk1Xsbnd( ARYK__MXDIM, acb3->ldtw, acb3->udtw, ARYK__MXDIM,
                             acb2->ldtw, acb2->udtw, ARYK__MXDIM, lx, ux,
                             &acb3->dtwex, status );
                  if( *status == SAI__OK ){
...
/* Apply the differential pixel index shifts to the new array section to
   convert its pixel indices to match the template (ary2) array. */
         aryk1Sft( acb1->ndim, sft, acb3, status );
...
/* If an error occurred, then report context information and call the error
   tracing routine. */
   if( *status != SAI__OK ){
      errRep( " ", "arykSsect: Error obtaining an array section using an existing"
              " section as a template.", status );
      aryk1Trace( "arykSsect", status );
   }
