...
*     values being retained, then a call to arykReset should be made
...
/* Local variables: */
   ...
   char type[ARYK__SZTYP+1];   /* Array numeric type */
...
/* Check the full type for validity. */
   aryk1Vftp( ftype, sizeof(type), type, &cmplx, status );

/* Import the array identifier. */
   acb = (AryACB *) aryk1Impid( ary, 1, 0, 1, status );

/* Check that TYPE access to the array is available. */
   aryk1Chacc( acb, "TYPE", status );

/* Set the new array type. */
   aryk1Stp( type, cmplx, acb, status );

/* If an error occurred, then report context information and call the error
   tracing routine. */
   if( *status != SAI__OK ){
      errRep( " ", "arykStype: Error setting new (full) type for an array.",
              status );
      aryk1Trace( "arykStype", status );
   }
