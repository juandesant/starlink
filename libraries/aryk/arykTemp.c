...
*     to other routines (e.g. arykNew or arykCopy) to produce a
...
/* Obtain a free slot in the PCB. */
   pcb = (AryPCB *) aryk1Ffs( ARYK__PCBTYPE, status );
   if( *status == SAI__OK ){

/* Create a temporary array placeholder object, storing a locator to it in
   the PCB. */
      dummy = 0;
      aryk1Temp( "ARRAY", 0, &dummy, &pcb->loc, status );
...
/* Get an external identifier fo the PCB, in the form of an AryPlace
   pointer. */
         *place = (AryPlace *) aryk1Expid( (AryObject *) pcb, status );

/* If an error occurred, then release the PCB slot. */
      } else {
         pcb = aryk1Rls( (AryObject *) pcb, status );
      }
   }
...
/* If an error occurred, then report context information and call the error
   tracing routine. */
   if( *status != SAI__OK ){
      errRep( " ", "arykTemp: Error obtaining placeholder for temporary array.",
              status );
      aryk1Trace( "arykTemp", status );
   }
