...
*  Notes:
*     -  The symbolic constant ARYK__SZTYP should be used for declaring the
...
/* Copy the numeric type string to the output argument. */
      aryk1Extyp( dcb, type, status );
   }

/* If an error occurred, then report context information and call the error
   tracing routine. */
   if( *status != SAI__OK ){
      errRep( " ", "arykType: Error obtaining the numeric type of an array.",
              status );
      aryk1Trace( "arykType", status );
   }
}
