...
*  Synopsis:
*     arykUnlock( Ary *ary, int *status );

*  Description:
*     This function ensures that the current thread does not have a
*     lock of any type on the supplied array. See arykLock.
*
*     The array must be locked again, using arykLock, before it can
...
*     arykLock, arykUnlock and arykLocked.
*     - Attempting to unlock an array that is not locked by the current
*     thread has no effect, and no error is reported. The arykLocked
...
/* Attempt to unlock the specified array. Test status first so that we know
   ...
   if( *status == SAI__OK ) {
      (void) aryk1DCBLock( acb->dcb, 3, 0, status );
   }

/* If an error occurred, then report context information and call the
   error tracing routine. */
   if( *status != SAI__OK ) {
      errRep( "ARY_UNLOCK_ERR", "arykUnlock: Error unlocking an array.",
              status );
      aryk1Trace( "ary1Unlock", status );
   }
}
